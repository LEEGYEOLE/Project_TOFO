...
public class FileManager {
	...
	public static String doFilerename(String pathname, String filename) {
    	...
    	s += System.nanoTime();	//밀리세컨드는 ㄴㄴ 같은 파일 만날 수 있기 때문, 10의 -9승
    	s += fileExt;
    	
    	try {
	    	File f1=new File(pathname+File.separator+filename);	//원래파일명
	    	File f2=new File(pathname+File.separator+s);	//바꿀파일명
	    	f1.renameTo(f2);	//동일한 파일명 있으면 안바뀜
	...
	/**
	 * 파일 삭제
	 * @param pathname 파일이 저장된 경로 (real path, not web path)
