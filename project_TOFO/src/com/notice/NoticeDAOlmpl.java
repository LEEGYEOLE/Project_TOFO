...
public class NoticeDAOlmpl implements NoticeDAO {
	...
	public int insertNotice(NoticeDTO dto) {
		...
		try {
			...
			sql = "INSERT INTO notice(num,  userId, subject, content, saveFilename, originalFilename, filesize, notice) VALUES (notice_seq.NEXTVAL, ?, ?, ?, ?, ?, ?, ?)";
	...
	public int dataCount(String condition, String keyword) {
		...
		try {
			// 조인안하면 이름 검색이 안된다.
			sql = "SELECT NVL(COUNT(*), 0) FROM notice b JOIN member m ON b.userId = m.userId"; // 이름을 member1이 가지고 있기
																									// 때문에
			if (condition.equalsIgnoreCase("created")) { // created이면~
				keyword = keyword.replace("-", ""); // condition은 컬럼명
				sql += " WHERE TO_CHAR(created, 'YYYYMMDD') = ?"; // 날짜검색
			} else if (condition.equalsIgnoreCase("userName")) {
				sql += " WHERE INSTR(userName, ?)=1"; // 앞부분쪽 보겠다는 뜻
			} else {
				sql += " WHERE INSTR(" + condition + ",?) >=1 "; // condition은 컬럼명
			}
	...
	public List<NoticeDTO> listNotice(int offset, int rows, String condition, String keyword) {
		...
		try {
			...
			if (condition.equalsIgnoreCase("created")) {
				...
				sb.append("WHERE INSTR(userName, ?)=1 "); // 앞부분쪽 보겠다는 뜻
			} else {
				sb.append("WHERE INSTR(" + condition + ",?) >=1 ");// condition은 컬럼명
			}
	...
	public NoticeDTO readBoard(int num) {
		...
		try {
			...
			sql = "SELECT num , b.userId , userName, subject, content, created, hitCount FROM notice b JOIN member m ON b.userId=m.userId WHERE num= ?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, num);
			rs = pstmt.executeQuery();
	...
	public NoticeDTO preReadNotice(int num, String condition, String keyword) {
		...
		try {
			if (keyword != null && keyword.length() != 0) {
				...
				sb.append("  SELECT num, subject FROM notice b JOIN member m ON b.userId = m.userId ");
				 if(condition.equalsIgnoreCase("created")) {
